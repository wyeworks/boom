defmodule Support.FakeMailer do
  ...
  defimpl Swoosh.Email.Recipient, for: PID do
    # Swoosh wont accept a raw PID in the to field, smudge it into the correct
    # format for unpacking in `deliver!/1`.
    def format(pid) do
      {inspect(pid), pid}
    end
  end

  # Overrides the `deliver!/1` function.
  #
  ...
  # in our case because we are catching -> sending mail, meaning tests need
  ...
  def deliver!(email) do
    ...
    # `name` will default to `""` when not specified in the email creation.
